...
### - 技術的特徴
  ...
  - **APIによるチャット内容の取得・投稿(`/mypage/chatroom`内)**<br>
    データ量が多いチャット内容の取得・頻繁に投稿されるチャットの処理もWEBサーバーで処理すると負荷が大きくなりがちなため、チャットの取得・投稿は別途API用サーバーを立てて行っています。<br>サーバー側のフレームワークには、各種コード・ドキュメントを自動生成でき、HTTPとgRPC両方のトランスポートの設定ができるGoa v3を使用しました。Goaはv2とv3で大きく仕様が変わっており、v3に関する情報が少ない難点がありますが、Go公式でGoaのDSLに関するドキュメントが公開されているため、比較的用意に実装が可能でした。<br>Swaggerを記述した上でgo-swaggerによりコードを生成する方法も検討しましたが、転職活動を開始するまでに時間がかかり過ぎると判断し、今回は見送りました。
  - **コンテナ内での作動(Docker)**<br>
